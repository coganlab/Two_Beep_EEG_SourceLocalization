...
def EOG_check(raw2, ica):
    ...
    '''
    eog_epochs = mne.preprocessing.create_eog_epochs(raw2, baseline=(-0.5, -0.1))
...
def EOG_annot(raw2, ica):
    ...
    onsets = eog_events[:, 0] / raw2.info['sfreq'] - 0.25
...
def BEM(SUBJECTS_DIR):
    ...
    mne.viz.plot_bem(subject=subj, subjects_dir=SUBJECTS_DIR, orientation='coronal', show=True, show_indices=True,
                     mri='T1.mgz', show_orientation=True)
    mne.bem.make_watershed_bem(subject=subj, subjects_dir=SUBJECTS_DIR, overwrite=True, volume='T1', atlas=False,
                               gcaatlas=False, preflood=None, show=False, copy=False, T1=None, brainmask='ws.mgz',
                               verbose=None)
...
def covariance_raw():
    ...
    epochs.save(pathlib.Path('/mnt/d/Aidan/' + subj + '/Epochs') / 'epochs_for_source_epo.fif',
                overwrite=True)  # to do co-registration, have to do file path so gui.coreg can take as input
...
def fsa_average(src, SUBJECTS_DIR):
    ...
    src_morph = mne.morph_source_spaces(src_from=src, subject_to='fsaverage', subjects_dir=SUBJECTS_DIR,
                                        surf='inflated')
